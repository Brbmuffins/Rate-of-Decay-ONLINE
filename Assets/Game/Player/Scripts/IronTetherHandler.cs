...
public class IronTetherHandler : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        _line.enabled       = false;
        // Use URP-compatible unlit shader so the line is visible in URP/HDRP.
        // "Sprites/Default" renders as invisible (or pink) in URP.
        Shader urpUnlit = Shader.Find("Universal Render Pipeline/Particles/Unlit");
        if (urpUnlit == null) urpUnlit = Shader.Find("Unlit/Color"); // fallback for non-URP
        Material lineMat = new Material(urpUnlit != null ? urpUnlit : Shader.Find("Sprites/Default"));
        lineMat.SetColor("_BaseColor", new Color(0.3f, 0.6f, 1f, 0.9f));
